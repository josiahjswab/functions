...
const SortAndCompare = (props) => {
  ...
  // Prevent result text on initilization of component.
  ...
  useEffect(() => {
    if (init) {
      setResultA(
        `List A: ${dbIsMissing.length} out of ${countAB[0]} are not in the other list.`
      )
      setResultB(
        `List B: ${testIsMissing.length} out of ${countAB[1]} are not in the other list.`
      )
    }
    setInit(true)
  }, [dbIsMissing])
  ...
  return (
    <div id="container_sort_compare">
      <div className="flex-column">
        ...
        <h2>{resultA}</h2>
        <ol>
          {dbIsMissing.length > 0
            ? dbIsMissing.map((item) => <li>{item}</li>)
            : null}
        </ol>
      </div>
      ...
      <div className="flex-column">
        ...
        <h2>{resultB}</h2>
        <ol>
          {testIsMissing.length > 0
            ? testIsMissing.map((item, index) => (
